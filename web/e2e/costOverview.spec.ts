...
test('should navigate to drilldown page', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { level: 2 }).or(page.getByText('命名空间', { exact: false })).first()
...
test('drilldown entry shows resource dimension selector and namespaces', async ({ page }) => {
  ...
  await expect(
    page.getByText('选择命名空间开始下钻').or(page.getByText('资源维度')).first()
  ).toBeVisible({ timeout: 15000 });
  await expect(page.getByText('资源维度').or(page.getByText('算力')).first()).toBeVisible();
...
test('storage dimension drilldown shows storage_class or pvc', async ({ page }) => {
  ...
  await expect(page.getByText('命名空间', { exact: false }).first()).toBeVisible({ timeout: 10000 });
  await expect(
    page.getByText('存储类').or(page.getByText('PVC')).or(page.getByText('子资源')).or(page.getByText('成本')).first()
